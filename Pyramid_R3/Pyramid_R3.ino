...
void setup()
{
  ...
  ConfAudio();
  AudioPlay(1, 0x1f); // file 1 level 1f
...
void loop() // run over and over
{
  ...
  if (Serial.available())
  {
    ...
    switch (a)
    {
      ...
      case 'a': Serial.println(F("play file 2")); AudioPlay(2, 0x1f); break;
  ...
  if ((!((millis() / 1000) % 3)))
  {
    ...
    colorWipe(LedStrip.Color(AcZ > 4000 ? 255 : 0, AcY > 4000 ? 255 : 0, AcX > 4000 ? 255 : 0), 50);
    GpsSerial.listen();
  }
  ...
  GsmSerial.println(putcmd);
  {
    ...
    while (millis() < start + 15000)  //wait 15 sec
    {
      if (GsmSerial.available())
      {
        ...
        TmpBuffer[i++] = a;
        if (i > 10) {
          if (!strcmp(TmpBuffer - 3, "*FTPGET")) start = millis() - 10000; //wait 15-10=5 sec
        }
